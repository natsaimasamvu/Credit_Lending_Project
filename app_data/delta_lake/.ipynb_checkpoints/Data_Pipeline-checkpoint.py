...
logger_app.logger.info('Initialising spark session setting context parameters')
...
def bronze(config):
    ...
    logger_app.logger.info('Reading in all data sources')
    data_frames = []
    ...
    logger_app.logger.info('Persisting data to bronze layer in delta format')
    index = 0
    for target, meta_data in config["storage_layer"]["bronze"].items():
        helper_functions.write_data(spark, data_frames[index], meta_data)
        index += 1
        
    logger_app.logger.info('Read write successful')    
    return data_frames
    
def silver(datasets, config):
    #Client transform 
    logger_app.logger.info('Starting silver layer transformations in the silver layer')
    ...
    logger_app.logger.info('Transformations completed successfully')
    
    datasets = [clients_silver_df, collaterals_silver_df, stocks_silver_df]
    
    #Persist the data to bronze layer
    logger_app.logger.info('Persisting data to the silver layer')
    index = 0
...
def gold(datasets, config):
    #Perform agregations
    logger_app.logger.info('Starting gold layer transformations')
    ...
    logger_app.logger.info('Showing dataset to be written to PostgreSQL DB')
    collateral_status_df.show()
    
    logger_app.logger.info('Transformations completed successfully')
    
    #Write to gold storage layer 
    logger_app.logger.info('Persisting data to the gold layer')
    helper_functions.write_data(spark, collateral_status_df, config["storage_layer"]["gold"]["collateral_status"])
    
    logger_app.logger.info('Initialising JDBC connection')
    logger_app.logger.info('Writting Gold data to PostgreSQL DB')
    (collateral_status_df.write
     ...
     .save())
    logger_app.logger.info('Write to PostgreSQL DB via JDBC connection successful')
    

def pipeline_run():
    # Read the configuration file as a dictionary
    logger_app.logger.info('Reading the config file')
    ...
    logger_app.logger.info('Application run successful')
pipeline_run()